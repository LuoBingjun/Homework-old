...
int gen_dstE()
{
    return ((((icode) == (I_RRMOVL) || (icode) == (I_IADDL)) & (cond)) ? 
      (rb) : ((icode) == (I_IRMOVL) || (icode) == (I_ALU)) ? (rb) : (
...
{
    return (((icode) == (I_RRMOVL) || (icode) == (I_ALU) || (icode) == 
        (I_LEAVE)) ? (vala) : ((icode) == (I_IRMOVL) || (icode) == 
        (I_RMMOVL) || (icode) == (I_MRMOVL) || (icode) == (I_IADDL)) ? 
      (valc) : ((icode) == (I_CALL) || (icode) == (I_PUSHL)) ? -4 : ((icode)
         == (I_RET) || (icode) == (I_POPL)) ? 4 : 0);
}
